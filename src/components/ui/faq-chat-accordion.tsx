  ...
  return (
    <div className={cn("p-4", className)}>
      <Accordion.Root
        ...
        collapsible
        value={openItem || ""}
        onValueChange={(value: any) => setOpenItem(value)}
      >
        {data.map((item) => (
          <Accordion.Item
              ...
              <Accordion.Trigger className="flex w-full items-center justify-start gap-x-4">
                ...
                <span
                  className={cn(
                    ...
                    openItem === item.id.toString() && "text-slate-200"
                  )}
                >
                  {openItem === item.id.toString() ? (
                    <FaMinus className="h-5 w-5 text-slate-200" />
                  ) : (
                    <BsPlus className="h-5 w-5 text-slate-200" />
                  )}
                </span>
              </Accordion.Trigger>
            </Accordion.Header>
